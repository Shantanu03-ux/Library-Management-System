...
class Main(object):
    def __init__(self,master):
        ...
        def displayStatistics(evt):
            ...
            print(count_books)
            self.lbl_book_count.config(text='Total : ' + str(count_books[0][0]) + ' books of library ')
            #self.lbl_member_count.config(test='Total Member :' + str(count_members[0][0]))
        ...
        def displayBooks(self):
            ...
            def bookInfo(evt):
                ...
                self.list_details.insert(0, " Author : " + book_info[0][2])
                self.list_details.insert(0, " Page : " + book_info[0][3])
                self.list_details.insert(0, " Language : " + book_info[0][4])
            ...
            # self.tabs.bind('<Button Release-1>', displayBooks)
        ...
        lbl_list = Label(list_bar, text='Sort By', font='times 16 bold', fg='#2488ff')
        ...
        image_bar=Frame(centerRightFrame,width=450,height=350)
        ...
        self.img_library=PhotoImage(file='icons/op.PNG')
        self.lblImg=Label(image_bar,image=self.img_library)
        self.lblImg.grid(row=1,column=0)
        ...
        #close connction
        self.icons = PhotoImage(file='icons/addbook.png')
        self.clcon = Button(topFrame, text='close Connection', image=self.icons, compound = LEFT, font='arial 12 bold',command = self.clCON)
        self.clcon.pack(side=LEFT, padx=10)

        # return book
        self.icon = PhotoImage(file='icons/ret.png')
        self.book = Button(topFrame, text='Return Book', image=self.icon, compound=LEFT, font='arial 12 bold', command = self.returnBook)
        self.book.pack(side=LEFT, padx=10)


        self.tabs = ttk.Notebook(centerLeftFrame, width=900, height=660)
        self.tabs.pack()
        self.tab1_icon = PhotoImage(file='icons/lms.png')
        self.tab2_icon = PhotoImage(file='icons/stat.png')
        self.tab1 = ttk.Frame(self.tabs)
        self.tab2 = ttk.Frame(self.tabs)
        self.tabs.add(self.tab1, image=self.tab1_icon, compound=LEFT)
        ...
        # statistics
        self.lbl_book_count = Label(self.tab2, text=" Welc ", pady=20, font='verdana 14 bold')
        ...
        self.lbl_taken_count = Label(self.tab2, text=" Welcome ", pady=20, font='verdana 14 bold')
    ...
    def listBooks(self):
        ...
        if value == 1:
            albooks = cur.execute("SELECT  * FROM books").fetchall()
            self.list_books.delete(0,END)
